...
import { useHexVaultRounds } from "@/features/hex/hooks/useHexVaultRounds";
...
export default function VaultCalendar({ userId }: { userId?: string }) {
  ...
  // Determine selected month range and fetch rounds for that range
  const monthStart = new Date(selectedRange.year, selectedRange.month, 1);
  const monthEnd = endOfMonth(monthStart);
  const startDateStr = format(monthStart, "yyyy-MM-dd");
  const endDateStr = format(monthEnd, "yyyy-MM-dd");

  const { data: rounds = [] } = useHexVaultRounds({
    userId: userId ?? "unauthenticated",
    startDate: startDateStr,
    endDate: endDateStr,
  });

  // Map rounds by date to match previous "answers[date]" usage
  const answers = useMemo(() => {
    ...
    for (const r of rounds) {
      map[r.date] = {
        liveScore: r.liveScore ?? 0,
        guessedWords: r.guessedWords ?? [],
        updatedAt: r.updatedAt ?? null,
        isRevealed: r.isRevealed ?? null,
        isTopRank: r.isTopRank ?? null,
      };
    }
    return map;
  }, [rounds]);
