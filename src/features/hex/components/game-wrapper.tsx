...
export default function GameWrapper({
  ...
  const { mutate: setAnswer } = useHexUserRoundMutation();
  ...
  function submitAnswer() {
    ...
    if (userRound?.guessedWords?.includes(inputWord)) {
    ...
    setAnswer({
      userId: userId ?? "unauthenticated",
      date,
      guessedWords: newGuessed.join(","),
      isTopRank,
      isRevealed: userRound?.isRevealed ?? false,
      isTopRankWhileLive: isLive
        ? isTopRank
        : (userRound?.isTopRankWhileLive ?? null),
      liveScore: isLive
      ...
      <div className="grid-rows-[auto_0.5fr_1fr]px-4 grid h-full w-full py-6">
        ...
        <div className="flex w-full items-center justify-center">
          <HexKeyboard
            ...
            onChange={onKeyDown}
            isDisabled={!!userRound?.isRevealed}
          />
        </div>
      </div>
      <WordListCard
